...
- コマンド：`/read1`（1枚引き）、`/read3`（3枚引き）、`/hexa`（ヘキサグラム）、`/celtic`（ケルト十字）
  - `/read3` はスリーカード（3枚固定）で、1枚目=過去、2枚目=現在、3枚目=未来の時間軸。時間スケール指定がない場合は前後3か月の出来事を想定し、流れ・傾向・気づきを示す。出力は見出しを付けず、`《カード》：` 行が3つ並び、箇条書きは未来（3枚目）のみ最大3点、断定を避けた提案ベースでまとめる。
  - `/love1` `/love3` は旧コマンドとして互換対応しています。

スリーカード（/read3）の出力イメージ（概念例）:

《カード》：カップの3（正位置）
過去の場面で、周囲との関わりに温かさが残っているようです。そこで得た安心感がいまの選択にも影響していそうです。
《カード》：ソードの8（逆位置）
現在は視点を少し変えるだけで、縛りをほどける余地があります。状況の枠組みを言葉にしてみると、整理が進みそうです。
《カード》：ワンドのペイジ（正位置）
- 気になる誘いがあれば、まず話を聞いてみると小さなきっかけになりそうです。
- 手軽に始められる準備を一つ決めて、肩慣らしをしてみてください。
- 迷ったときは「なぜ気になるか」を言葉にしてから、次の手順を選ぶと動きやすそうです。
