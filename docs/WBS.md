...
- [ ] [~] T1-03: `help`/`terms`/`support` の内容整備（文面・URL・窓口）
- [ ] [~] T1-04: 入力ガード（空入力・長文・絵文字だらけ等でも崩れない）
- [ ] [~] T1-05: 例文（質問の例）をテーマ別に最小セット
- [ ] T1-06: 多重送信対策（連打・同時リクエストのキュー/ロック）
- [ ] T1-07: “戻る/やり直し”導線（テーマ選択に戻る、質問を変える等）
...
- [ ] [~] T3-04: 会話状態（theme/question/spread）を安全に保持（メモリ or 永続）
