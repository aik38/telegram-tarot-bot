...
- [ ] T8A-02: 結果の見せ方（PR7代替: 出力フォーマット整備＝結論→理由→行動、読みやすさ最優先）
    - 出力冒頭を「《カード》：カード名（正/逆位置） - 役割ラベル」に統一（例：「《カード》：ペンタクルの6（逆位置） - メインメッセージ」）
    - 見出し・題名（「引いたカード」「メインメッセージ」など）は付けない
    - 結論段落と箇条書きが重複しないよう統合し、結論は箇条書き側でまとめる
    - 箇条書きの本数・長さの量感は現行を維持（増やさない）
